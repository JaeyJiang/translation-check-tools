    ...
    def start_filter(self, file_name):
        ...
        for index in range(base_length):
            ...
            if StringUtils.string_is_no_empty(sheet_1.cell(index,
                ...
                self.__list_report_data[index].append([index,
                                                       "",
                                                       "此条翻译Key为空".format(
                                                           sheet_1.cell(index,
                                                                        0).value)])
                ...
                if index > 0:
                    ...
                    if StringUtils.string_is_no_empty(compare_string) is False:
                        ...
                        if 0 in self.__list_switch:
                            self.__list_report_data[index].append([col_index_compare,
                                                                   compare_string,
                                                                   "{0:s}:存在漏翻".format(language_head)])
                        ...
                        self.__list_report_data[index].append([col_index_compare,
                                                               compare_string,
                                                               "{0:s}:存在Excel内置报错内容: \"{1:s}\"".format(language_head,
                        ...
                        if (1 in self.__list_switch) and (
                            ...
                            self.__list_report_data[index].append([col_index_compare,
                                                                   compare_string,
                                                                   "{0:s}:标点符号问题,半角/全角不一致或数量不一致".format(
                                                                       language_head)])
                        ...
                        if (2 in self.__list_switch) and (
                            ...
                            self.__list_report_data[index].append([col_index_compare,
                                                                   compare_string,
                                                                   "{0:s}:占位符问题(%),译文中的%后的标记与基准列的不一致".format(
                                                                       language_head)])
                        ...
                        if (5 in self.__list_switch) and (
                            ...
                            self.__list_report_data[index].append([col_index_compare,
                                                                   compare_string,
                                                                   "{0:s}:html/xml标签问题,译文中的标签存在不一致".format(
                                                                       language_head)])
                        ...
                        if (6 in self.__list_switch) and (
                            ...
                            self.__list_report_data[index].append([col_index_compare,
                                                                   compare_string,
                                                                   "{0:s}:颜色标签[xxxxxx],译文中的标签存在不一致".format(
                                                                       language_head)])
                        ...
                        if (9 in self.__list_switch) and (
                            ...
                            self.__list_report_data[index].append([col_index_compare,
                                                                   compare_string,
                                                                   "{0:s}:译文长度相对原文长度的比值大于指定的阈值".format(
                                                                       language_head)])
                            # 检查文本头尾是否有空格
                        if (10 in self.__list_switch) and (
                                ...
                                for i in range(len(keys)):
                                    ...
                                    if key in sheet_1.cell(index, 0).value:
                                        ...
                                        if result[0] is False:
                                            ...
                                            self.__list_report_data[index].append([col_index_compare,
                                                                                   compare_string,
                                                                                   "{0:s}:该Key进行了文本长度配置检查,该文本超过配置中指定长度:{1:d}\t 当前长度:{2:d}".format(
            ...
            while f_col_num < len(self.__select_compare_col) + 3:
                ...
                for col_content in compare_content:
                    ...
                    if f_row_num > 0 and f_col_num > 2:
                        if self.__select_compare_col[f_col_num - 3] == col_content[0]:
                            sheet.write(f_row_num, f_col_num,
                                        sheet_data.cell(compare_row, col_content[0]).value, style3)
                f_col_num += 1
            f_row_num += 1
